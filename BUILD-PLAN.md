...
*Status 2026-07-22: unified tags/local matching deployed (`5eb5320`). Evidence: 318 non-live tests pass; the 60-pair multilingual relation gate reports AUC 1.000, zero unrelated pairs over 0.5, and batch-invariant vectors; migration 015 and the pinned 279 MB model warmed on production; 17,947 real claims rebuilt to 6,373 events/12,253 occurrences; 5,390 canonical tag rows (297 source + 5,093 category) and 34 vectors are live; public query, hard-filter listing, structured event detail, calendar page, and semantic ICS checks return 200; WKO has 35 claims -> 27 canonical events/6 future occurrences. The 6,373 v5 LLM enrichments are queued but safely parked by the kind-wide credit circuit breaker because OpenRouter is at 130.0037/130 credits; inferred tags/language/stated-price/venue will converge automatically after credit is added. No post-deploy failures appeared during the observation window.*

*Status 2026-07-24: the event-first search and completeness increment is deployed through `e39c16d`. One structured call now composes title, provenance, multilingual confidence-bearing tags, weekdays, price, and event-scale intent; discovery returns one row per event before its limit, and filtered calendars reuse the same hard membership model. Generic public hydration appends evidence-backed exact facts, while schema-current enrichment supplies confidence-labeled price, participant estimates, and 6-12 activity/format/audience/atmosphere/setting tags when facts cannot be established. Multi-concept semantic intent uses exact-or-stronger evidence, focused nonlinear relatedness, monotonic harmonic coverage, and bounded joint context, so no requested concept can substitute for another; secondary price/scale preferences refine that intent unless callers explicitly change `importance`. The live MCP schema includes executable examples, closed top-level inputs, explicit date-only calendar guidance, and rejects ambiguous semantic calendar membership. Independent MCP tasks for balls, WKO/startup, and Thursday/Friday salsa repeatedly found contract defects that were repaired generically; the final calls passed with complete price/scale coverage for their accepted sets and exact calendar threshold transfer. The 60-pair semantic gate remains at AUC 1.000 with zero unrelated pairs over 0.5; the final full non-live suite passed 343 tests (53 live tests deselected). Production API and three workers are healthy; final corpus and acceptance evidence is recorded in `docs/superpowers/plans/2026-07-24-search-completeness-mcp.md`.*
