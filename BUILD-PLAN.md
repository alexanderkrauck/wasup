...
*Status 2026-07-24: the event-first search and completeness increment is deployed through `fa45f7c`. One structured call now composes title, provenance, multilingual confidence-bearing tags, weekdays, price, and event-scale intent; discovery returns one row per event before its limit, and filtered calendars reuse the same hard membership model. Generic public hydration appends evidence-backed exact facts, while v6 enrichment supplies confidence-labeled price and participant estimates when facts cannot be established. The live MCP schema includes executable examples and rejects ambiguous semantic calendar membership. Independent MCP tasks for WKO/startup and Friday salsa found contract defects that were repaired generically (explicit calendar thresholds, occurrence-level weekday filtering, and honest low-confidence crowd ranges). The 60-pair semantic gate remains at AUC 1.000 with zero unrelated pairs over 0.5; the final full non-live suite passed 334 tests (53 live tests deselected). Production API and three workers stayed healthy while the one-time atomic canonical rebuild filled the resolver cache; final corpus coverage evidence is recorded in `docs/superpowers/plans/2026-07-24-search-completeness-mcp.md`.*
